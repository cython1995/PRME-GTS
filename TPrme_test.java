...
public class TPrme_test {
	...
	static double  XT_L[][]=new double [MAX_POINT][NUM_DIM];
     ...
     double calc_d(int u,int lc,int li,int t){
    	 return alpha*calc_dp(u,li)+beta*calc_dt(t,li)+(1-alpha-beta)*calc_ds(lc,li);
     }
     double calc_d_multiple(int u,int lc,int li,int t){
    	 return calc_dp(u,li)*calc_dt(t,li)*calc_ds(lc,li);
     }
     ...
     void load_trained_result(String file_name){
         ...
         try {
			       ...
			       for(int i=0;i<TIME_PERIODS;i++){
			    	   String []DIM4=temp.split(" ");
			    	   for(int j=0;j<NUM_DIM;j++){
			    	       XT_T[i][j]=Double.parseDouble(DIM4[j]);
			    	   }
			    	   temp=br.readLine();
			       }
			       for(int i=0;i<total_point;i++){
			    	   String []DIM5=temp.split(" ");
			    	   for(int j=0;j<NUM_DIM;j++){
			    		   XT_L[i][j]=Double.parseDouble(DIM5[j]);
			    	   }
			    	   temp=br.readLine();
			       }
			      /* for(int i=0;i<total_user;i++){
     ...
     void do_test(int id,int all){
    	 ...
    	 int u=c.user_id;
    		 ...
    		 for(int i=0;i<total_point;i++){
    			 double d=calc_d(u,lc,i,c.timestamp);
     ...
     void test(){
    	 ...
    	 System.out.println("除以记录数Total precision="+precision1);
    	 System.out.println("除以记录数Total recall="+recall1);
    	 System.out.println("-------------------------------------------------------------");
	...
	public static void main(String[] args) {
      ...
      TPrme_test prme=new TPrme_test();
      prme.load_trained_result("d://result_gts_gowalla.txt");
	...
	//  prme.load_test_data("d://test_set_big.txt");
	  ...
	  {
	   //创建一个printWriter类的实例，其构造函函数是一个File对象
	   PrintWriter fout = new PrintWriter("d://RESULT_PRME_GT1.txt");
